\chapter{Search for MSSM $\PH \to \Ph\Ph \to \Pgt\Pgt \Pqb\Pqb$}
\label{chap:Hhh}
...
the \ac{MSSM} $\Pphi\to\Pgt\Pgt$ analysis discussed in
...
are simply those with the highest $\pt$, there is a high probability that these 
will not be the correct jets originating from the $\Pqb$ quarks. 
...
(b) channels. Note that the effect of the b-tag scale factor correction between
data and \ac{MC} is not included in this figure.}
\label{fig:Hhhnjets}
\end{figure} 
...
As in the $\HToTauTau$ analyses, a cut on transverse mass of $m_{\text{T}} < 30
\,\GeV$ is applied, which was found to be optimal in rejection of $\WJets$ and
the increased $\ttbar$ in this analysis. Finally, events are categorised depending 
on whether zero, one or two of the selected jets pass the
...
equation \ref{eq:bjetrelation}. The mass of both tau leptons is kept
constant at $m_{\Pgt}$.  
...
in variables such as $m_{\Pgt\Pgt}$ and $m_{\Pqb\Pqb}$ where the signals look very 
...
Note that the narrow width approximation is valid for all mass points
considered in this analysis, since the resolution in $m_{\PH}$ is always at least
$10\,\GeV$ worse than the theoretical width of the $\PH$ in the signal samples.
...
same-sign data, which is found to be almost entirely QCD with negligible contributions
from other backgrounds and hence does not have the
...
size of the subtraction of the $\ttbar$ contamination in the embedded sample, is
included. 
...
above, this analysis is not so sensitive to the \ac{SM} Higgs boson, and hence
...
allows a small amount of the low $\tan\beta$ phase space to be excluded. Note
that limits going as low as $\tan\beta=1$ are shown and not any lower since
this is the region of the cross-section calculations which was fully validated
by the \ac{LHCHXSWG} at the time of writing this thesis.
