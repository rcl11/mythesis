...
As described in detail in section~\ref{sec:mssmbenchmarks}, there are many popular
...
$m_{A}-\tan\beta$ space ruled out for such \ac{MSSM} scenarios, focus shifts more 
...
additional b-jets to form the $\Ph\to\Pqb\Pqb$ part. 
Thus a large amount of the methods discussed in chapters 
...
must contain a minimum of two jets in order to reconstruct the $\Ph\to\Pqb\Pqb$
...
$30~\GeV$ to $20~\GeV$. The higher jet $\pt$ cut in the other analyses was found
to increase sensitivity by reducing backgrounds containing softer jets than in
signal. Howeber, for a signal of the type
...
$\pt$ cut on the jets would cut out too much signal in this analysis. 
...
indicates the number of jets which are b-tagged, where the b-tagging condition
...
or do not pass the medium \ac{CSV} working point. This means that if 
the two jets selected to reconstruct the $\Ph\to\Pqb\Pqb$ are simply those with the highest
$\pt$, as is conventionally used in many analyses, there is a high probability
that these will be the correct jets
originating from the $\Pqb$ quarks. From studies using the generator level
information of the signal \ac{MC}, it was found that a higher success rate for selecting the
...
In signal events of all $m_{\PH}$ hypotheses, the reconstructed $m_{\Pqb\Pqb}$ and
...
the categories. It can also be seen that the signal peaks around $125~\GeV$ in
...
\begin{equation}
m_{\Ph}^{2} = p_{b_{1}}^{2} + p_{b_{2}}^{2,fit} + 2p_{b_{1}}p_{b_{2}}^{fit} 
...
using a \ac{MC} sample in different bins of $\eta$ and $\pt$ \cite{}. 
...
is greatly improved. Figure \ref{fig:kinfitsignalmasses} shows the
$m_{\PH}$ distribution with the kinematic fit (denoted $m_{\PH}^{kinfit}$) 
...
Since the \ac{CSV} values are not directly used in this analysis except to make
...
%check if the y axis label is really correct for this plot
\begin{figure}
\begin{center}
\subfloat[]{
    \includegraphics[width=0.5\textwidth]
...
Note that in all of these results, unlike in the \ac{MSSM} analysis, no possible
